...
func LocalTest(ctx context.Context) error {
	fmt.Println("Running Local test suite")
	...
	for _, goos := range oses {
		for _, goarch := range arches {
			...
			output := golang.
				...
				WithWorkdir("/src/api/provider/local").
				WithExec([]string{"go", "test", ".", "-v"})

			_, err = output.Stdout(ctx)
			if err != nil {
				fmt.Printf("❌ failed (local) %s %s\n", goos, goarch)

				return err
			}
			fmt.Printf("✔ test case passed (local) %s %s\n", goos, goarch)
		}
	}
